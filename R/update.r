...
setMethod("update", signature(object="MFCLFrq"), function(object, years, fisheries, multiplier, quantity, ...){
  ...
  for(ff in fisheries){
    cateffpen(object)[is.element(cateffpen(object)$year, years) & is.element(cateffpen(object)$fishery, ff) & cateffpen(object)[,quantity]!=-1, quantity] <-
      cateffpen(object)[is.element(cateffpen(object)$year, years) & is.element(cateffpen(object)$fishery, ff) & cateffpen(object)[,quantity]!=-1, quantity] * multiplier[which(fisheries==ff)]
  }
