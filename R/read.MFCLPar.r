...
read.MFCLBiol <- function(parfile, first.yr=1972){
  ...
  slot(res, "mat")      <- FLQuant(scan(parfile, skip=grep("# percent maturity ",  par), n=nagecls, quiet=quiet), dimnames=dims_age)
...
read.MFCLRegion <- function(parfile, first.yr=1972) {
  ...
  dca <- aperm(array(dca, dim=c(nregions, nagecls, nseasons, nregions), 
                     dimnames=list(from=as.character(1:5), age=as.character(1:16), period=as.character(1:4), to=as.character(1:5))),
               c(1,4,2,3))
...
read.MFCLSel <- function(parfile) {
  ...
  slot(res, 'av_q_coffs')   <- FLQuant(aperm(array(scan(parfile, skip=grep("# average catchability coefficients", par)+1,n=nagecls, quiet=quiet), 
                                                   dim=c(nseasons, nagecls/nseasons,1,1,1)),c(2,3,4,1,5)), dimnames=dims1)
  slot(res, 'ini_q_coffs')  <- FLQuant(aperm(array(scan(parfile, skip=grep("# initial trend in catchability coefficients ", par)+1,n=nagecls, quiet=quiet), 
                                                   dim=c(nseasons, nagecls/nseasons,1,1,1)),c(2,3,4,1,5)), dimnames=dims1)
  slot(res, 'q0_miss')      <- FLQuant(aperm(array(scan(parfile, skip=grep("# q0_miss", par)+1, n=nagecls, quiet=quiet), 
                                                   dim=c(nseasons, nagecls/nseasons,1,1,1)),c(2,3,4,1,5)), dimnames=dims1)  
