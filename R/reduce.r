...
setMethod("reduce", signature(obj='MFCLFrq'),
  ...
  cateffpen(frq2) <- cateffpen(obj)[is.element(cateffpen(obj)$fishery, fisheries),]
  ## Need to do the same thing for length and weight comp
  lnfrq(frq2) <- lnfrq(obj)[is.element(lnfrq(obj)$fishery, fisheries),]
  wtfrq(frq2) <- wtfrq(obj)[is.element(wtfrq(obj)$fishery, fisheries),]
  # renumber the fisheries in the freq
  #freq(frq2)$fishery <- freq(frq2)$fishery-3   ### this is a hack - be careful
  cateffpen(frq2)$fishery <- rep(1:length(fisheries), table(cateffpen(obj)$fishery)[fisheries])
  lnfrq(frq2)$fishery <- rep(1:length(fisheries), table(lnfrq(obj)$fishery)[fisheries])
  wtfrq(frq2)$fishery <- rep(1:length(fisheries), table(wtfrq(obj)$fishery)[fisheries])

  lf_range(frq2)["Datasets"] <- nrow(cateffpen(frq2))
